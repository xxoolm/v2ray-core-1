...
func TestShadowsocksSimpleObfsHttp(t *testing.T) {
	tcpServer := tcp.Server{
		MsgProcessor: xor,
	}
	dest, err := tcpServer.Start()
		...
		Outbound: []*core.OutboundHandlerConfig{
			{
				ProxySettings: serial.ToTypedMessage(&shadowsocks.ClientConfig{
					...
					Plugin:     "obfs-local",
					PluginOpts: "obfs=http;host=shadow.v2fly.org",
				}),
			},
		},
	}
...
func TestShadowsocksSimpleObfsTLS(t *testing.T) {
	tcpServer := tcp.Server{
		MsgProcessor: xor,
	}
	dest, err := tcpServer.Start()
		...
		Outbound: []*core.OutboundHandlerConfig{
			{
				ProxySettings: serial.ToTypedMessage(&shadowsocks.ClientConfig{
					...
					Plugin:     "obfs-local",
					PluginOpts: "obfs=tls;host=shadow.v2fly.org",
				}),
			},
		},
	}
